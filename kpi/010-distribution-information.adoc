...
Distribution information allows the user to understand what formats are available, where to get it and who to contact. 
...
|10.1: The `MD_Format` section is present.
|1

|10.2: The `specification` element in the 'MD_Format' section has an `Anchor` with a resolvable HTTP URL.
|1

|10.3: The `organisationName` element in the `MD_Distributor` section is present. 
|1

|10.4: The `electronicMailAddress` in the `MD_Distributor` section is present. 
|1

|10.5: One to many `MD_DigitalTransferOptions` options are present. 
 |1
...
* Include the relevant WMO data formats in the `MD_Format` sections with a link to the specification of the data format. 
* Include all relevant URLs in the `MD_DigitalTransferOptions` section for accessing the data.  
* A distributor contact does not have to be the same as the main point of contact, principal investigator and should always have
